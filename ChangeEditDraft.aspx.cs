...
public partial class Change: System.Web.UI.Page   
{
    ...
    public DataTable GetChangeTasks(string changeee)
    {
        string constr = ConfigurationManager.ConnectionStrings["ConString"].ConnectionString;
        SqlConnection con = new SqlConnection(constr);
        SqlCommand cmd = new SqlCommand(@"select ROW_NUMBER() Over ( Order By ct.task_name ) 'Task_ID', ct.task_name, ct.task_description, e.employee_name 'Task_Implementer', 
            e.Email 'Implementer_Email', FORMAT(ct.task_start,'dd/MM/yyyy hh:mm:ss.000') 'task_start', FORMAT(ct.task_end,'dd/MM/yyyy hh:mm:ss.000') 'task_end', ct.implementer 'Implementer_ID'  
            from Change_new c inner join change_tasks ct on c.change_ID = ct.change_ID inner join employee e on ct.implementer = e.employee_ID inner join task_status 
            cs on ct.status = cs.status_ID where c.change_id = '" + changeee + @"'");
        SqlDataAdapter sda = new SqlDataAdapter();
        cmd.Connection = con;
        sda.SelectCommand = cmd;
        DataTable dt = new DataTable();
    ...
    public DataTable GetChangeView(string changeee)
    {
        string constr = ConfigurationManager.ConnectionStrings["ConString"].ConnectionString;
        SqlConnection con = new SqlConnection(constr);
        SqlCommand cmd = new SqlCommand(@"select  e1.employee_name 'Approver1', e2.employee_name 'Approver2',c.* from Change_new c
            left outer join employee e1 on e1.employee_ID = c.approver1 
            left outer join employee e2 on e2.employee_ID = c.approver2 
            where c.change_id = '" + changeee + @"'");
        SqlDataAdapter sda = new SqlDataAdapter();
        cmd.Connection = con;
        sda.SelectCommand = cmd;
        DataTable dt = new DataTable();
    ...
    public void GetDelegatedApprover()
    {
        string constr = ConfigurationManager.ConnectionStrings["ConString"].ConnectionString;
        SqlConnection con = new SqlConnection(constr);
        SqlCommand cmd = new SqlCommand(@"select c.temp_employee_id + '#' + e.email 'Employee_ID', e.Employee_name  from change_delegation c inner join employee e on c.temp_employee_id = e.employee_id where convert(date,GETDATE(),103) between convert(date,c.start_date,103) and convert(date,c.end_date,103)");
        SqlDataAdapter sda = new SqlDataAdapter();
        cmd.Connection = con;
        sda.SelectCommand = cmd;
        DataTable dt = new DataTable();
        ...
        if (dt.Rows.Count > 0)
            dd_Approver2.Items.Insert(0, new ListItem(dt.Rows[0][0].ToString(), dt.Rows[0][1].ToString()));
        else
    ...
    protected void btn_SaveDraft_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string changeQuery = @"update Change_new
                set Title = '" + txt_Title.Text + @"', Submit_Date = getdate(), Submit_DateTime = getdate(), Approver1 = '" + approverEmail[0] + @"'
                ,Approver2 = '" + approver2Email[0] + @"', Impact_ID = '" + dd_Impact.SelectedValue + @"', Urgency_ID = '" + dd_Urgency.SelectedValue + @"'
                ,Priority_ID = '" + dd_Priority.SelectedValue + @"', Stage_ID = '1', Risk_ID = '" + dd_Risk.SelectedValue + @"'
                ,Type_ID = '" + dd_Category.SelectedValue + @"', SubType_ID = '" + dd_SubCategory.SelectedValue + @"', Start_Date = CONVERT(datetime, '" + datetime1 + @"', 103)  
                ,End_Date = CONVERT(datetime, '" + datetime2 + @"', 103), Reason_ID = '" + dd_ReasonForChange.SelectedValue + @"'
                ,Change_Plan = '" + txt_Details.Text.Replace("'", "") + @"',Backout_Plan = '" + txt_BackoutPlan.Text.Replace("'", "") + @"'
                ,Modify_DateTime = getdate(), Modify_User = '" + Session["UserID"].ToString() + @"', Status = '0'
                , ref_incident_no = '" + txt_refIncident.Text + "',Vendor_Involved = '" + dd_VendorInvolved.SelectedValue 
            ...
            string logQuery = @"insert into change_log (change_id, status, status_by, status_datetime, status_description) values ('" + changeID
                + @"', '1', '" + Session["UserID"].ToString() + @"', getdate(), '" + Session["Username"].ToString() + " Saved the Change as Draft" + @"')";
            ...
            int x = 0; int y = 0; int p = 0; int q = 0; int z = 0;
            string TaskQuery = ""; string serviceQuery = "";
            ...
            try
            {
                ...
                foreach (DataRow item in tasks.Rows)
                {
                    ...
                    queries.Add(TaskQuery);  //z = cmd.ExecuteNonQuery();
                }

                queries.Add(delattachmentquery); //cmd.CommandText = delattachmentquery; //x = cmd.ExecuteNonQuery();
                ...
                queries.Add(delServices); //cmd.CommandText = delServices;
                ...
                queries.Add(logQuery);                 //cmd.CommandText = logQuery;
    ...
    protected void btn_SaveNew_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string changeQuery = @"update Change_new set Title = '" + txt_Title.Text + @"', Submit_Date = getdate(), Submit_DateTime = getdate()
                , Approver1 = '" + approverEmail[0] + @"', Approver2 = '" + approver2Email[0] + @"', Impact_ID = '" + dd_Impact.SelectedValue + @"'
                , Urgency_ID = '" + dd_Urgency.SelectedValue + @"', Priority_ID = '" + dd_Priority.SelectedValue + @"', Stage_ID = '3'
                , Risk_ID = '" + dd_Risk.SelectedValue + @"', Type_ID = '" + dd_Category.SelectedValue + @"', SubType_ID = '" + dd_SubCategory.SelectedValue + @"'
                , Start_Date = CONVERT(datetime, '" + datetime1 + @"', 103), End_Date = CONVERT(datetime, '" + datetime2 + @"', 103)                                                      
                , Reason_ID = '" + dd_ReasonForChange.SelectedValue + @"', Change_Plan = '" + txt_Details.Text.Replace("'", "") + @"'
                , Backout_Plan = '" + txt_BackoutPlan.Text.Replace("'", "") + @"', Modify_DateTime = getdate()
                , Modify_User = '" + Session["UserID"].ToString() + @"', Status = '1', ref_incident_no = '" + txt_refIncident.Text 
                + "',Vendor_Involved = '" + dd_VendorInvolved.SelectedValue + "' where change_ID = '" + changeID + @"'";

           
            if (dd_Urgency.SelectedValue == "05")
            {
                changeQuery = @"update Change_new set Title = '" + txt_Title.Text + @"', Submit_Date = getdate(), Submit_DateTime = getdate()
                    ,Impact_ID = '" + dd_Impact.SelectedValue + @"', Urgency_ID = '" + dd_Urgency.SelectedValue + @"', Priority_ID = '" + dd_Priority.SelectedValue 
                    + @"' ,approver1 = NULL, approver2 = NULL, Stage_ID = '3', Risk_ID = '" + dd_Risk.SelectedValue + @"', Type_ID = '" + dd_Category.SelectedValue + @"'
                    , SubType_ID = '" + dd_SubCategory.SelectedValue + @"', Start_Date = CONVERT(datetime, '" + datetime1 + @"', 103)  
                    , End_Date = CONVERT(datetime, '" + datetime2 + @"', 103), Reason_ID = '" + dd_ReasonForChange.SelectedValue + @"'
                    , Change_Plan = '" + txt_Details.Text.Replace("'", "") + @"', Backout_Plan = '" + txt_BackoutPlan.Text.Replace("'", "") + @"'
                    , Modify_DateTime = getdate(), Modify_User = '" + Session["UserID"].ToString() + @"',  Status = '1', ref_incident_no = '" 
                    + txt_refIncident.Text + "', Vendor_Involved = '" + dd_VendorInvolved.SelectedValue + "' where change_ID = '" + changeID + @"'";
            }
            ...
            int x = 0; int y = 0; int z = 0; int p = 0; int q = 0;

            try
            {
                queries.Add(changeQuery); //cmd.ExecuteNonQuery();

                queries.Add(delTasks);  //cmd.CommandText = delTasks;
                foreach (DataRow item in tasks.Rows)
                {
                    TaskQuery = @"INSERT INTO Change_tasks( Change_ID, Task_Name, Task_Description, Implementer, Status, Task_start, Task_end)
                ...
                string emailBody = @" <p>Dear <b>Approver</b>,<br /> a Change has been initiated in CIS by <b>" + Session["Username"].ToString() + 
                ...
                foreach (DataRow item in tasks.Rows)
                    emailBody += @"<tr>
                                    <td> <p>" + item["Task_Implementer"].ToString() + @" </p> </td>
                ...
                conHRSmart.Open();
                emailQuery = @"insert into Email_Logs (To_Address, CC_Address, Email_Subject, Email_Body, Sent_Flag, From_Application, From_Module, Confirmation_Msg, 
                    From_UserCode, Created_Date) values ('" + emailIDs + "', '" + emailCC + "', 'For your Approval, Change ID: " + changeID + @"', '" + emailBody
                  + @"', 'N', 'CIS', 'Change', '', '', GETDATE());";
...
//            string datetime1 = txt_datefrom.Text + " " + addZero(TimeSelector3.Hour.ToString()) + ":" + addZero(TimeSelector3.Minute.ToString()) + ":" 
//                + addZero(TimeSelector3.Second.ToString()) + ":000 " + TimeSelector3.AmPm;
//            string datetime2 = txt_datefrom.Text + " " + addZero(TimeSelector4.Hour.ToString()) + ":" + addZero(TimeSelector4.Minute.ToString()) + ":" 
//                + addZero(TimeSelector4.Second.ToString()) + ":000 " + TimeSelector4.AmPm;
...
//                                                           ,Task_start
//                                                           ,Task_end)
//                                                     VALUES ('" + changeID + "','" + item["Task_Name"].ToString() + "','" + item["Task_Description"].ToString() + "','" + item["Implementer_ID"].ToString() + "','0',CONVERT(datetime, '" + item["Task_Start"].ToString() + @"', 103) ,CONVERT(datetime, '" + item["Task_End"].ToString() + @"', 103) )";
...
//                        //SMTP Email
//                        Email.SendEmail(approverEmail[1] + ";" + approver2Email[1] + ";" + implementerEmail[1] + ";" + requestorEmail[1], "", "For your Approval, Change ID: " + changeID, emailBody);
...
//                                                                        values ('" + approverEmail[1] + ";" + approver2Email[1] + ";" + implementerEmail[1] + ";" + requestorEmail[1] + "', 'For your Approval, Change ID: " + changeID + @"', '" + emailBody + @"', 'N', 'CIS', 'Change', '', '" + Session["UserID"].ToString() + @"', GETDATE());
//                                                                        ";
